...
def prepare_samples(df):
    ...
    """
    return list(zip(df['Amino Acid Sequence'], df['3Di String']))
...
def prepare_amino_to_fold_seek_samples(dataset_path, task_token, max_length, max_samples, random_seed, logging):
    df = pd.read_csv(dataset_path)

    if max_samples < len(df):
        # randomly select max_samples rows
        df = df.sample(max_samples, replace=True)
    ...
    for sequence, label in samples:
        new_samples.append(
            (task_token, sequence[:max_length-2], [f'3Di_{char}' for char in label[:max_length-2]], 'null', np.full(2, 1))
        )
